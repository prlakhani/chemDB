...
from excel.util import import_objects_from_excel, import_relationships_from_excel, \
    export_objects_to_excel, export_relationships_to_excel, \
    render_excel


def start(request, template="excel/start2.html", extra_context=None):
...
def import_object(request, template="excel/import_object2.html", extra_context=None,
                  return_action=start):
    if extra_context is None:
        extra_context = {}
        ...
        if model_for_import and file_contents:
            objects_added, objects_updated, objects_not_changed, objects_not_changed_error = \
                import_objects_from_excel(
                    model_for_import, file_contents, request=request)
...
def import_relation(request, template="excel/import_relation2.html", extra_context=None,
                    return_action=start):
    if extra_context is None:
        extra_context = {}
    if request.method == 'POST':
        ...
        if model_for_import and file_contents:
            relationships_added, relationships_not_added, relationships_not_added_error = \
                import_relationships_from_excel(
                    model_for_import, field_name, clean, file_contents, request=request)
        extra_context['relationships_added'] = relationships_added
        extra_context['relationships_not_added'] = relationships_not_added
        extra_context[
            'relationships_not_added_error'] = relationships_not_added_error
